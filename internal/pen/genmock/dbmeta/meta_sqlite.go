...
func LoadSqliteMeta(db *sql.DB, sqlType, sqlDatabase, tableName string) (DbTableMeta, error) {
	...
	for i, v := range cols {
		...
		notNull := strings.Contains(colDDLLower, "not null")
		isPrimaryKey := strings.Contains(colDDLLower, "primary key")
		isAutoIncrement := strings.Contains(colDDLLower, "autoincrement")
		defaultVal := ""
		columnLen := int64(-1)
