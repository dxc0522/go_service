...
func (s *Schema) MergeProperty(p Property) error {
	...
	for _, e := range s.Properties {
		if e.JsonFieldName == p.JsonFieldName && !PropertiesEqual(e, p) {
			return errorx.New(fmt.Sprintf("property '%s' already exists with a different type", e.JsonFieldName))
		}
	}
...
func GenerateGoSchema(sref *openapi3.SchemaRef, path []string) (Schema, error) {
	...
	if schema.AllOf != nil {
		...
		if err != nil {
			return Schema{}, errorx.Wrap(err, "error merging schemas")
		}
	...
	if extension, ok := schema.Extensions[extPropGoType]; ok {
		...
		if err != nil {
			return outSchema, errorx.Wrapf(err, "invalid value for %q", extPropGoType)
		}
	...
	if t == "" || t == "object" {
		...
		if len(schema.Properties) == 0 && !SchemaHasAdditionalProperties(schema) {
			...
			for _, pName := range SortedSchemaKeys(schema.Properties) {
				...
				if err != nil {
					return Schema{}, errorx.Wrap(err, fmt.Sprintf("error generating Go schema for property '%s'", pName))
				}
			...
			if schema.AdditionalProperties != nil {
				...
				if err != nil {
					return Schema{}, errorx.Wrap(err, "error generating type for additional properties")
				}
		...
		case "array":
			...
			if err != nil {
				return Schema{}, errorx.Wrap(err, "error generating type for array")
			}
	...
	trySkipPointer(schema, &outSchema)
	return outSchema, nil
}

func trySkipPointer(in *openapi3.Schema, out *Schema) {
	if shouldSkipOptionalPointer(out.GoType) {
		out.SkipOptionalPointer = true
	}
}

func shouldSkipOptionalPointer(goType string) bool {
	return goType[:2] == "[]" || goType == "json.RawMessage"
}
...
func MergeSchemas(allOf []*openapi3.SchemaRef, path []string) (Schema, error) {
	...
	for _, schemaOrRef := range allOf {
		...
		if ref != "" {
			...
			if err != nil {
				return Schema{}, errorx.Wrap(err, "error converting reference path to a go type")
			}
		}

		schema, err := GenerateGoSchema(schemaOrRef, path)
		if err != nil {
			return Schema{}, errorx.Wrap(err, "error generating Go schema in allOf")
		}
		...
		for _, p := range schema.Properties {
			...
			if err != nil {
				return Schema{}, errorx.Wrap(err, "error merging properties")
			}
		}
		...
		if schema.HasAdditionalProperties {
			if outSchema.HasAdditionalProperties {
				...
				if schema.AdditionalPropertiesType.TypeDecl() != outSchema.AdditionalPropertiesType.TypeDecl() {
					return Schema{}, errorx.New("additional properties in allOf have incompatible types")
				}
			} else {
	...
	if err != nil {
		return Schema{}, errorx.Wrap(err, "unable to generate aggregate type for AllOf")
	}
	return outSchema, nil
}
...
func GenStructFromAllOf(allOf []*openapi3.SchemaRef, path []string) (string, error) {
	...
	for _, schemaOrRef := range allOf {
		...
		if ref != "" {
			...
			objectParts = append(objectParts,
				fmt.Sprintf("\t%s `json:\",inline\"`", goType))
		} else {
