...
func (pd *ParameterDefinition) IsPointer() bool {
	if shouldSkipOptionalPointer(pd.Schema.GoType) {
		return false
	}
	return !pd.Required
}
		...
		if responseRef.Value != nil {
			...
			for _, contentTypeName := range sortedContentKeys {
				...
				if contentType.Schema != nil {
					...
					if err != nil {
						return nil, errorx.Wrap(err, fmt.Sprintf("Unable to determine Go type for %s.%s", o.OperationId, contentTypeName))
					}
					...
					if contentType.Schema.Ref != "" {
						...
						if err != nil {
							return nil, errorx.Wrap(err, "error dereferencing response Ref")
						}
...
func OperationDefinitions(swagger *openapi3.T) ([]OperationDefinition, error) {
	...
	for _, requestPath := range SortedPathsKeys(swagger.Paths) {
		...
		for _, opName := range SortedOperationsKeys(pathOps) {
			...
			if err != nil {
				return nil, errorx.Wrap(err, "error generating body definitions")
			}
...
func GenerateBodyDefinitions(operationID string, bodyOrRef *openapi3.RequestBodyRef) ([]RequestBodyDefinition, []TypeDefinition, error) {
	...
	for contentType, content := range body.Content {
		...
		if err != nil {
			return nil, nil, errorx.Wrap(err, "error generating request body definition")
		}
		...
		if bodyOrRef.Ref != "" {
			...
			if err != nil {
				return nil, nil, errorx.Wrap(err, fmt.Sprintf("error turning reference (%s) into a Go type", bodyOrRef.Ref))
			}
...
func GenerateTypesForOperations(t *template.Template, ops []OperationDefinition) (string, error) {
	...
	if err != nil {
		return "", errorx.Wrap(err, "error generating types for params objects")
	}

	err = t.ExecuteTemplate(w, "request-bodies.tmpl", ops)
	if err != nil {
		return "", errorx.Wrap(err, "error generating request bodies for operations")
	}
	...
	//if err != nil {
	//	return "", errorx.Wrap(err, "error generating additional properties boilerplate for operations")
	//}
	...
	err = w.Flush()
	if err != nil {
		return "", errorx.Wrap(err, "error flushing output buffer for server interface")
	}
...
func GenerateChiServer(t *template.Template, operations []OperationDefinition) (string, error) {
	...
	if err != nil {
		return "", errorx.Wrap(err, "error generating server interface")
	}

	err = t.ExecuteTemplate(w, "chi-middleware.tmpl", operations)
	if err != nil {
		return "", errorx.Wrap(err, "error generating server middleware")
	}

	err = t.ExecuteTemplate(w, "chi-handler.tmpl", operations)
	if err != nil {
		return "", errorx.Wrap(err, "error generating server http handler")
	}

	err = w.Flush()
	if err != nil {
		return "", errorx.Wrap(err, "error flushing output buffer for server")
	}
